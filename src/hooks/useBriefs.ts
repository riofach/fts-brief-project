...
export const useBriefs = (enabled = true, options?: {
  staleTime?: number;
...
export const useBrief = (briefId?: string, enabled = true) => {
  ...
  const validBriefId = briefId && typeof briefId === 'string' && briefId.length > 0 && !briefId.includes('[object Object]') ? briefId : undefined;
...
export const useUpdateBriefStatus = () => {
  ...
  return useMutation({
    mutationFn: async ({
      briefId,
      status,
    }: { briefId: string; status: UpdateBriefStatusRequest['status'] }): Promise<Brief> => {
      const response = await api.put(`/briefs/${briefId}`, { status });
      return response.data.data;
    },
    onSuccess: (updatedBrief) => {
      // Update the brief in all relevant queries
      queryClient.setQueryData<Brief>(
        queryKeys.briefs.detail(updatedBrief.id),
        updatedBrief
      );
